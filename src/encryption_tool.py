...
def generate_keys(algorithm, key_file=None, content=None):
    ...
    # RSA key generation or loading
        ...
        if key_file:
            # If key files are provided, read the public and private keys from the files
            ...
            # Otherwise, generate a new RSA key pair (1024-bit keys)
...
def main():
    ...
    if args.encrypt:
        ...
        if args.algorithm == "caesar":
            try:
                encrypted_file = caesar_cipher_binary(args.input, 10)  # Encrypt with Caesar cipher (shift 10)
        ...
        # AES encryption
            ...
            try:
                ...
                encrypted_file = AES_encryption(args.input, key)  # Encrypt using AES encryption
        ...
        # OTP encryption
            ...
            try:
                ...
                key = generate_keys("otp", content=content)  # Generate OTP key
                encrypted_data, otp_key = OTP_encryption(content, key)  # Encrypt using OTP

                with open(args.output, 'wb') as encrypted_file:
        ...
        # RSA encryption
            ...
            try:
                public_key, _ = generate_keys("rsa", args.key)  # Load the public RSA key
                encrypted_file = RSA_encrypt_file(args.input, public_key)  # Encrypt using RSA encryption
    ...
    # Decryption process
        ...
        # OTP decryption
            ...
            try:
                with open(args.input, 'rb') as encrypted_file:
                    encrypted_data = encrypted_file.read()  # Read the encrypted file
        ...
        # RSA decryption
            ...
            try:
                _, private_key = generate_keys("rsa", args.key)  # Load the private RSA key
